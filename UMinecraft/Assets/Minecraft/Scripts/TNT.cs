...
public class TNT : MonoBehaviour
{
    ...
    public static readonly float FUSE_TIME = 6.0f;
  ...
  //  string _EmissionColor = "_EmissionColor";
    ...
    public void Explode()
    {
            ...
            for (int x = -blastRadius; x <= blastRadius; x++)
            {
                for (int y = -blastRadius; y <= blastRadius; y++)
                {
                    for (int z = -blastRadius; z <= blastRadius; z++)
                    {
                        ...
                        chunk = ChunkManager.Instance.GetNearestChunk(pos);
        ...
        ObjPool.GetComponent<ParticleSystem>(explosionEffect, center, Quaternion.identity, 0.5f);
